...
我们的抽取只从 `getResultItem()` 读一个产出，所以一条 Create 粉碎配方
（3 个产出，其中两个 `chance: 0.75`）会只回来 1 个产出、且没有概率 ——
是**不完整**而不是被标记。`sequenced_assembly` 那种多步配方嵌套了一整串子配方，
看起来可读，实际漏掉了大部分成本。这两件都是待办工作，不是视图器问题：
JEI/EMI 能揭示多个产出，但都不提供 `processingTime`；
嵌套序列需要专门的适配器，或者诚实地标 `opaque`。
