...
- `sequenced_assembly` 嵌套了一整串子配方。按现在的方式读，它会**看起来可读**、
  实际漏掉大部分原料成本，所以需要专门的适配器或者诚实标 `opaque`。目前没覆盖。
- **模组配方的机器名得靠 JEI。** Create 没有覆写 `getToastSymbol()`，
  ...
  注液/排液分散在 Spout 和 Item Drain 上）。JEI 的催化剂才是权威答案 ——
  这就是为什么第一个要接的视图器是 JEI 而不是 EMI。
- 能耗仍然全是 `null`。
