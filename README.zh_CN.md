...
dependencies {
    ...
    compileOnly 'dev.craftgraph:craftgraph:0.1.1'
}
```
...
在真实的 1.21.1 实例上端到端验证过：索引 1290 条配方、3% 标记为读不懂、
主线程抽取 37ms、AI 客户端零配置连接成功。

| | |
|---|---|
| 桥接 Mod | 可用。104 个 JUnit 用例，都不需要启动 Minecraft |
...
**读不懂的配方是真实存在的，而且被计数。** 原版 1290 条里有 40 条标记为 `opaque`：
27 条锻造，13 条代码驱动的特殊合成（染色、地图复制、烟花、旗帜复制）。
想看你自己整合包按类型的分解，运行 `npm run inspect`。

那 27 条锻造值得单独说，因为玩家问得最多的正是它们（下界合金装备）。
`SmithingRecipe` 没有覆写 `getIngredients()`，继承的默认实现返回空列表，
三个槽位只能通过 `isTemplateIngredient` / `isBaseIngredient` / `isAdditionIngredient`
这三个谓词访问。标成 `opaque` 是诚实的答案，AI 也被告知要说「这条我读不懂」而不是
「这配方不需要材料」。

实测问「下界合金头盔怎么做」时的表现：AI 如实转达了「输入读不出来，这不代表它不需要原料」——
然后接着凭自己的训练数据补出了原版做法，并明确标注了「这不是从游戏数据里读到的」。
在原版上那恰好是对的；但在整合包里，KubeJS 脚本或别的模组可能改过这条配方，
同样一句话就会变成**理直气壮的错误答案**。
**`opaque` 防的是「静默的错误答案」，防不住模型用记忆把空缺补上。**
把锻造的材料读出来是可行的（遍历物品注册表去测那三个谓词，目前没做），
而它的价值比覆盖度数字看起来的要高。
