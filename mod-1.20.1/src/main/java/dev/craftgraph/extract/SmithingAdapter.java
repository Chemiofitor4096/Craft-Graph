...
/**
 * 锻造配方的适配器（**1.20.1 版**）：输入来自三个包私有字段（靠
 * {@code META-INF/accesstransformer.cfg} 变公开），产物按两个子类分别处理。
 *
 * <p>与 1.21.1 那份的差别只有一处、但很关键：**这个版本的 AT 用 SRG 名**
 * （Forge 1.20.1 的硬性要求），所以 {@code META-INF/accesstransformer.cfg} 里那六行
 * 长得像 {@code f_265949_} 而不是字段名。这里读的字段名（template/base/addition）两边一样 ——
 * 常量池里的名字由重映射负责，写代码时用的是官方名。
 *
 * <h2>为什么需要它</h2>
 *
