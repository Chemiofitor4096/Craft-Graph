...
改完之后我才在真实包上量：**它改变了 39% 的首选**（1,428 / 3,639 个有多个候选的物品），
而提交前我只对了**一个**案例（金苹果）。提交信息里也把它说得比实际窄。

这不好，而且它正是这个仓库反复警告的那类错误 —— 只不过这次犯在启发式上而不是字段上。
补救是留下 `npm run routes`：**改启发式之前先跑它看数字，别用一个案例代表全部。**

同一次量测也给出一个让人安心的结论：在我能定义的「怪」轴（首选路线含整合度 ≤1 的叶子）
上，改动前后是 4 → 5（3,639 里），基本中性 —— 它**确实**修掉了它针对的那一类
（`iron_block` 从「高炉烧生铁块」回到「9 铁锭合成」），也没有明显制造新的怪路线。

另外，这条改动的性质值得说准：原来的 `inputs.length * 10` 想表达的本来就是
「输入种类越少越简单」，只是被 3×3 网格的槽位数量测错了。
所以它是**修正度量**，不是改偏好 —— 那 39% 的变动就是原本的偏好在被正确执行。
