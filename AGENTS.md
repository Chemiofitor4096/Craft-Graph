...
| Windows 批处理（`.bat`）里**不要写中文** | cmd.exe 按**控制台代码页**（中文 Windows 是 GBK）解析 .bat 的**字节**，UTF-8 的中文既变乱码、又**破坏解析本身**（报「不是内部或外部命令」，而且指向随机的半截片段）；`chcp 65001` 写在文件里也来不及 —— 那时文件自己的字节已经被读错了。批处理**只写 ASCII**（**`rem` 注释也不例外** —— GBK 解读下某些字节对会被当成命令分隔符，注释行照样报错，实测过），中文交给它调用的 Node/PowerShell 输出；另外 `.bat` **必须是 CRLF**（LF 结尾会让 cmd 把命令挤在一起，报指向随机半截片段的错误） |
