...
#                     也可以直接双击仓库根的 live-check.bat：它会先查 jar / Node / 游戏，再跑这一条
...
| Windows 批处理（`.bat`）里**不要写中文** | cmd.exe 按**控制台代码页**（中文 Windows 是 GBK）解析 .bat 的**字节**，UTF-8 的中文既变乱码、又**破坏解析本身**（报「不是内部或外部命令」，而且指向随机的半截片段）；`chcp 65001` 写在文件里也来不及 —— 那时文件自己的字节已经被读错了。批处理**只写 ASCII**（**`rem` 注释也不例外** —— GBK 解读下某些字节对会被当成命令分隔符，注释行照样报错，实测过），中文交给它调用的 Node/PowerShell 输出（`live-check.bat` 就是这么做的）；另外 `.bat` **必须是 CRLF**（LF 结尾会让 cmd 把命令挤在一起，报指向随机半截片段的错误） |
| GitHub Actions 的 **job id 不能带点** | 约束是 `[A-Za-z_][A-Za-z0-9_-]*`。写成 `mod-1.20.1` 会让**整个 workflow 文件失效**，而症状离原因很远：run 立刻失败、**一个 job 都没有**、workflow 名字变成文件路径 —— 本地拿任何 YAML 解析器看却都是好的。版本号放显示名（`name:`）里，job id 用 `mod-forge` 这种 |
| 改了 `core/` 的代码 | 两个 mod 版本都编一遍（`mod` 与 `mod-1.20.1`）。只跑一个构建就以为改对了，另一侧的症状要到别人装了那个版本才出现 |
