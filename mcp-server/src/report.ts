...
import { computeFieldCoverage } from "./coverage.js";
...
/**
 * 机器数这一栏的状态 —— **判定只有这一份**，markdown 与 HTML 各自措辞。
 *
 * <h2>为什么判定不能各渲染器各写一遍</h2>
 *
 * 「手工」和「台数未知」是两种完全不同的原因：前者是原版设计
 * （工作台**没有耗时字段**），后者是**我们的数据缺口**（读不到模组配方的耗时）。
 * 混起来的一次实测事故：同一份输出的机器表写「未知」、明细写「手工」——
 * 而填充机不是工作台。判定一旦分叉，两个渲染器就会把这两种原因说成不同的东西。
 */
export function machineStatus(node: PlanNode): "computed" | "manual" | "unknown" {
  if (node.machines != null) return "computed";
  return node.recipeType === "minecraft:crafting" ? "manual" : "unknown";
}
 ...
 *
 * <p>explorer.ts（HTML 渲染）也引用这一份 —— 同一个事实要从两个出口出去时，
 * 措辞只能有一处实现，否则两个出口迟早各说各话（这个项目里发生过两次）。
 */
export const SKIP_REASON: Record<"cycle" | "probabilistic" | "user_choice", string> = {
  cycle: "会构成循环依赖，已避开",
...
function renderPlanNode(store: RecipeStore, node: ProductionPlan["root"], depth: number, lines: string[]): void {
  ...
  if (node.recipeId) {
    ...
    // 措辞随介质，判定在 machineStatus —— 那里记着「手工 vs 未知」混写的事故
    const ms = machineStatus(node);
    const machineText =
      ms === "computed" ? `${node.machines} 台` : ms === "manual" ? "手工" : "台数未知（读不到耗时）";
