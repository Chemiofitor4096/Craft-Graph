...
export interface Recipe {
  ...
  opaque: boolean;
  /**
   * true 表示这条配方**本来就不产出物品**（燃料/配置类定义），而不是「产出读不到」。
   *
   * 两者都让 `outputs` 为空，但含义相反：前者是我们读清楚了，后者是我们没读懂。
   * 混为一谈会让 AI 把燃料配方说成「这条我读不懂」，而真相是「它不产出东西」。
   *
   * 出现时保证 `opaque === false` 且 `outputs` 为空（Mod 侧强制）。
   * 旧版 Mod 不发这个字段，所以是可选的。
   */
